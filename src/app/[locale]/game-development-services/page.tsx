...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const page = commercialPages[safeLocale]["game-development-services"];
  return buildCustomMetadata(locale, page.title, page.metaDescription, page.slug);
}

export default async function GameDevelopmentServicesPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const safeLocale = locale === "es" ? "es" : "en";
  return <CommercialLandingPage locale={safeLocale} page={commercialPages[safeLocale]["game-development-services"]} />;
}
